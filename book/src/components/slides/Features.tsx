...
export default function Features(props: FeaturesProps) {
  ...
  return (
    <div className="h-full w-full flex flex-col">
      ...
      {/* 3 floating browser cards — visible side-by-side, slight tilt + lift */}
      <motion.div
        initial="hidden"
        whileInView="show"
        ...
        {top.map((f, i) => {
          ...
          return (
            <motion.div
              ...
              whileHover={{ y: lift - 6, rotate: 0, scale: 1.02 }}
              style={{ transform: `translateY(${lift}px) rotate(${tilt}deg)` }}
              className="group rounded-2xl bg-navy-900/90 border border-white/10 shadow-slide overflow-hidden"
            >
              {/* Browser chrome */}
              ...
              <div className="p-5">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="h-display text-[22px] leading-tight text-white truncate">{f.title}</div>
                  </div>
                </div>
                ...
                <div className="mt-3 flex items-center justify-between">
                  ...
                  <span className="text-[12px] text-gold-400 group-hover:translate-x-1 transition">Voir →</span>
                </div>
              </div>
            </motion.div>
          )
      ...
      {/* The rest — compact horizontal pills under the cards */}
      {rest.length > 0 && (
        <motion.div
          initial="hidden"
          whileInView="show"
          ...
          {rest.map((f) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[15px] font-semibold text-white leading-tight truncate">{f.title}</div>
                <div className="text-[12px] text-navy-200 leading-snug truncate">{f.body}</div>
              </div>
            </motion.div>
          ))}
        </motion.div>
